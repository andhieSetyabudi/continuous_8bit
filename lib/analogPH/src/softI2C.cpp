...
// -----------------------------------------
//    The softI2C can be declared more than once,
...
//    and every softI2C object requires 59 bytes at the moment.
//
//============================================================================
...
softI2C::softI2C(uint8_t sdaPin, uint8_t sclPin, boolean pullups, boolean detectClockStretch)
{
  _sdaPin = sdaPin;
  _sclPin = sclPin;
  _pullups = pullups;
...
//
// The function to set the pins of Software I2C bus after create the object => softI2C ()
//

void softI2C::init()//(uint8_t sdaPin, uint8_t sclPin, boolean pullups, boolean detectClockStretch)
{
 // _sdaPin = sdaPin;
 // _sclPin = sclPin;
  _pullups = true;
...
void softI2C::beginTransmission(uint8_t address)
{
  ...
  _transmission = softI2C_NO_ERROR;
  ...
  if(i2c_start())
  {
    ...
    {
      _transmission = softI2C_ADDRESS_NACK;
    }
  }
  else
  {
    // If the bus was not okay, the scl or sda didn't work.
    _transmission = softI2C_OTHER;
  }
}
...
//
uint8_t softI2C::endTransmission(boolean sendStop)
{
  if(sendStop)
    i2c_stop();
  else
...
//
uint8_t softI2C::requestFrom(uint8_t address, uint8_t size, boolean sendStop)
{
  ...
  if(sendStop || _transmission != softI2C_NO_ERROR)
    i2c_stop();
  else
    i2c_repstart();
...
//
uint8_t softI2C::requestFrom(int address, int size, boolean sendStop)
{
...
size_t softI2C::write(uint8_t data)
{
  ...
  if( _transmission == softI2C_NO_ERROR)
  {
...
//
size_t softI2C::write(const uint8_t* data, uint8_t quantity)
{
...
//
size_t softI2C::write(char* data)
{
  int n = strlen(data);
...
int softI2C::readBytes(uint8_t* buf, uint8_t size)
{
  int data;
  int n;
...
//
int softI2C::readBytes(char * buf, uint8_t size)
{
...
//
int softI2C::readBytes(char * buf, int size)
{
...
//
void softI2C::setTimeout(long timeout)
{
  // 2017, fix issue #6.
...
//
void softI2C::printStatus( Print& Ser)
{
  Ser.println(F("-------------------"));
  Ser.println(F("softI2C Status"));
  Ser.println(F("-------------------"));
  Ser.print(F("  F_CPU = "));
  Ser.println(F_CPU);
  Ser.print(F("  sizeof(softI2C) = "));
  Ser.println(sizeof(softI2C));
  ...
  Ser.print(F("  softI2C_BUFSIZE = "));
  Ser.println(softI2C_BUFSIZE);
  ...
  for(int ii=0; ii<softI2C_BUFSIZE; ii++)
  {
    if(rxBuf[ii] < 16)
      Ser.print(F("0"));
...
//
boolean softI2C::i2c_start(void)
{
...
//
uint8_t softI2C::i2c_read(boolean ack)
{
  uint8_t res = 0;
